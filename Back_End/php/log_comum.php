...
include_once("config.php");
    ...
    if (!empty($_GET['search'])) {
        $data = $_GET['search'];
        $sql = "SELECT * FROM log WHERE (id_usuario LIKE '%$idUsuario%' OR nome LIKE '%$nome%') AND id_usuario = $idUsuario ORDER BY id_usuario DESC";
    } else {
        $sql = "SELECT * FROM log WHERE id_usuario = $idUsuario ORDER BY id_log DESC";
    }
